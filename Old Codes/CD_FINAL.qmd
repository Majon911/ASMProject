...
## Compare it with the attributes of gender and office (maybe graphically is a good approach) - \[ last part of the question\]
...
# Assuming 'gender_data_list' contains gender information for each network's communities
gender_data_list <- list(
  Network1_Community1 = data.frame(Community = c("Community1", "Community1", "Community1"),
                                   Gender = c("Male", "Male", "Female")),
  Network1_Community2 = data.frame(Community = c("Community2", "Community2", "Community2"),
                                   Gender = c("Female", "Male", "Male")),
  Network1_Community3 = data.frame(Community = c("Community3", "Community3", "Community3"),
                                   Gender = c("Female", "Male", "Male")),
  Network1_Community4 = data.frame(Community = c("Community4", "Community4", "Community4"),
                                   Gender = c("Female", "Male", "Male")),
  
)

# Function to create gender distribution plot for a community
plot_gender_distribution <- function(gender_data, community_num) {
  ggplot(gender_data, aes(x = Gender, fill = Gender)) +
    geom_bar(position = "fill", width = 0.6) +
    labs(title = paste("Distribution of Genders in Community", community_num),
         x = "Gender", y = "Proportion") +
    scale_fill_manual(values = c("blue", "pink"), name = "Gender",
                      labels = c("Male", "Female")) +
    theme_minimal()
}

# List to store individual plots for each community in each network
plots_list <- list()

# Generate plots for each community in each network
for (i in 1:length(gender_data_list)) {
  network_num <- substr(names(gender_data_list)[i], 8, 8)  # Extract network number
  for (j in 1:length(gender_data_list[[i]])) {
    community_num <- j
    # Create plot for each community
    plot <- plot_gender_distribution(gender_data_list[[i]][[j]], community_num)
    # Store each plot in the list
    plots_list[[paste("Network", network_num, "Community", community_num)]] <- plot
  }
}

# Combine and arrange plots for each network
combined_plots <- lapply(split(plots_list, names(plots_list)), function(plots) {
  do.call(grid.arrange, c(plots, ncol = 2))
})

# Display combined plots for each network
grid.arrange(grobs = combined_plots, ncol = 1)

```

```{r}
node_attributes <- V(work)$attribute_name
node_attributes
```
